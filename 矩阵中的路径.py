...
class Solution:
    ...
    def hasPathCore(self, matrix, rows, cols, row, col, path, pathLength, visited):
        ...
        visited[row * cols + col]:
            ...
            hasPath = self.hasPathCore(matrix, rows, cols, row, col - 1, path, pathLength, visited) or self.hasPathCore(
                matrix, rows, cols, row - 1, col, path, pathLength, visited) or self.hasPathCore(matrix, rows, cols,
                                                                                                 row, col + 1, path,
                                                                                                 pathLength,
                                                                                                 visited) or self.hasPathCore(
                matrix, rows, cols, row + 1, col, path, pathLength, visited)
            if not hasPath:
                pathLength -= 1
